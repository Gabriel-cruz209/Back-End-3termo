        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6 bg-gray-50 border-b border-gray-200">
                    <div class="flex justify-between items-center">
                        <div>
                            <h3 class="text-lg font-medium text-gray-900">Pedidos Registrados</h3>
                            <p class="mt-1 text-sm text-gray-600">{{ $pedidos->count() }} pedidos no sistema</p>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">R$ {{ number_format($pedido->valor_total, 2, ',', '.') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                        @if($pedido->status == 'aprovado')
                                        @elseif($pedido->status == 'pendente') bg-yellow-100
                                        @else text-red-800 @endif">
                                        {{ ucfirst($pedido->status) }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ ucfirst($pedido->metodo_pagamento) }}</td>
                                <td class="px-6 py-4 text-sm text-gray-500 max-w-xs truncate">{{ $pedido->observacoes ?: 'Nenhuma' }}</td>
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-4 text-center text-gray-500">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <a href="{{ route('pedidos.create') }}" class="mt-2 text-indigo-600 hover:text-indigo-500">Criar primeiro pedido</a>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
